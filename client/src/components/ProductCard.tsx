...
const ProductCard = (props: Item) => {
  ...
  const addToCart = async() => {
   ...
   await axios.post(upDateCartEndPoint,
    { "item": {
    "name": props.name,
    "seller":props.seller,
  ...
  return (
    <div >
      <div className='cart flex flex-col p-3 m-2  bg-white rounded-md hover:bg-gray-300  shadow-md'>
        ...
        <div className='seller-container px-4 pb-2 text-indigo-500'> Seller: &nbsp; {props.seller}</div>
        <div className='flex justify-between p-2 py-4 mb-2 relative'>
          <div className='quantity px-2  tex-indigo-500 '>Quantiy:  {quantity}  <button className='text-2xl absolute left-[6.5rem] top-[.7rem] border-2 border-gray-200 px-1' onClick={handleClick}>+</button> </div>
          <div className='type px-2 text-indigo-500 '>{props.type}</div>
        </div>

        <button className='p-2 w-max bg-indigo-300 hover:bg-indigo-400 hover:text-black mx-auto shadow-lg' onClick={addToCart} >Set to Cart</button>
